...
SOperatorInfo* createStreamSessionAggOperatorInfo(SOperatorInfo* downstream,
  ...
  int32_t code = initStreamAggSupporter(&pInfo->streamAggSup, pResBlock->info.rowSize,
...
typedef int32_t (*__get_value_fn_t)(void* data, int32_t index);
...
int32_t getSessionWindowEndkey(void* data, int32_t index) {
...
static bool isInWindow(SResultWindowInfo* pWin, TSKEY ts, int64_t gap) {
  int64_t tmpGap = ts - pWin->win.skey;
  if (tmpGap < 0) {
    tmpGap = -tmpGap;
  }
  if ( tmpGap < gap) {
...
static SResultWindowInfo* addNewSessionWindow(SArray* pWinInfos, TSKEY ts,
  ...
  SResultWindowInfo win = 
      {.pos.offset = -1, .pos.pageId = -1, .win.skey = ts, .win.ekey = ts};
...
SResultWindowInfo* getSessionTimeWindow(SArray* pWinInfos, TSKEY ts, int64_t gap,
  ...
  *pIndex = index;
  return addNewSessionWindow(pWinInfos, ts, index);
}

int32_t updateSessionWindowInfo(SResultWindowInfo* pWinInfo, TSKEY* pTs, int32_t rows,
    int32_t start, int64_t gap) {
  for (int32_t i = start; i < rows; ++i) {
    if (!isInWindow(pWinInfo, pTs[i], gap)) {
      return i - start + 1;
    }
    pWinInfo->win.skey = TMIN(pWinInfo->win.skey, pTs[i]);
...
void compactTimeWindow(SStreamSessionAggOperatorInfo* pInfo, int32_t startIndex, int32_t num,
  ...
  ASSERT(num < taosArrayGetSize(pInfo->streamAggSup.pResultRows) - 1);
  // Just look for the window behind StartIndex
  for (int32_t i = 0; i < num; i++) {
    SResultWindowInfo* pWinInfo = taosArrayGet(pInfo->streamAggSup.pResultRows, startIndex + 1);
    ...
    taosArrayRemove(pInfo->streamAggSup.pResultRows, startIndex + 1);
  }
}

static SArray* doStreamSessionWindowAggImpl(SOperatorInfo* pOperator,
  ...
  for(int32_t i = 0; i < pSDataBlock->info.rows; ) {
    ...
    winRows =
      updateSessionWindowInfo(pCurWin, tsCols, pSDataBlock->info.rows, i, pInfo->gap);
    ...
    if (winNum > 0) {
      compactTimeWindow(pInfo, winIndex, winNum, groupId, numOfOutput, pTaskInfo);
    }
    ...
    pos->groupId = groupId;
    pos->pos = (SResultRowPosition){.pageId = pResult->pageId, .offset = pResult->offset};
    *(int64_t*)pos->key = pResult->win.ekey;
    taosArrayPush(pUpdated, &pos);
    i += winRows;
  }
  return pUpdated;
...
static void doClearSessionWindows(SStreamAggSupporter* pAggSup, SOptrBasicInfo* pBinfo,
  ...
  for (int32_t i = 0; i < pBlock->info.rows; i += step) {
    ...
    SResultWindowInfo* pCurWin =
        getSessionTimeWindow(pAggSup->pResultRows, tsCols[i], gap, &winIndex);
    step = updateSessionWindowInfo(pCurWin, tsCols, pBlock->info.rows, i, gap);
...
static SSDataBlock* doStreamSessionWindowAgg(SOperatorInfo* pOperator) {
  ...
  if (pOperator->status == OP_RES_TO_RETURN) {
    ...
    if (pBInfo->pRes->info.rows == 0 ||
        !hasRemainDataInCurrentGroup(&pInfo->groupResInfo)) {
      doSetOperatorCompleted(pOperator);
      return NULL;
    }
    return pBInfo->pRes;
  ...
  while (1) {
    ...
    pUpdated = doStreamSessionWindowAggImpl(pOperator, pBlock);
  }
